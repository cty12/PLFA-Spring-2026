...
-- | Type variable parallel substitution
infixr 7 _⇒ˢ_
...
-- | Type variable single substitution
...
data _⊢_ {Δ : TyCtx} (Γ : Ctx Δ) : Type Δ → Set where
  `zero  : Γ ⊢ `Nat
  `_     : ∀ {A} → Γ ∋ A → Γ ⊢ A
  ƛ_     : ∀ {A B} → Γ , A ⊢ B → Γ ⊢ A ⇒ B
  ...
  Λ_     : ∀ {A} → ⇑ᶜ Γ ⊢ A → Γ ⊢ ∀̇ A
  _∙_    : ∀ {A} → Γ ⊢ ∀̇ A → (B : Type Δ) → Γ ⊢ A [ B ]ᵗ

-----------------------------------------------------------------------------
-- Substitutions used by β-rules
-----------------------------------------------------------------------------
...
-- | Type-variable renaming on terms
renameᵀ : ∀ {Δ Δ'} (ρ : Δ ⇒ʳ Δ') {Γ : Ctx Δ} {A : Type Δ}
  → Γ ⊢ A
  → renameCtx ρ Γ ⊢ renameᵗ ρ A
renameᵀ ρ `zero    = `zero
renameᵀ ρ (` x)    = ` (renameᵗ-∋ ρ x)
renameᵀ ρ (ƛ N)    = ƛ (renameᵀ ρ N)
renameᵀ ρ (L · M)  = renameᵀ ρ L · renameᵀ ρ M
renameᵀ ρ {Γ = Γ} (Λ N) = Λ (substEq (_⊢ _) (renameCtx-extᵗ-⇑ᶜ ρ Γ) (renameᵀ (extᵗ ρ) N))
...
-- | Term-variable parallel substitution
subst : ∀ {Δ} {Γ Δ' : Ctx Δ} {A : Type Δ}
  → Γ →ˢ Δ'
  → Γ ⊢ A
  → Δ' ⊢ A
subst σ `zero    = `zero
...
-- | Type-variable parallel substitution on terms
substᵀ : ∀ {Δ Δ'} (σ : Δ ⇒ˢ Δ') {Γ : Ctx Δ} {A : Type Δ}
  → Γ ⊢ A
  → substCtx σ Γ ⊢ substᵗ σ A
substᵀ σ `zero    = `zero
substᵀ σ (` x)    = ` (substᵗ-∋ σ x)
substᵀ σ (ƛ N)    = ƛ (substᵀ σ N)
substᵀ σ (L · M)  = substᵀ σ L · substᵀ σ M
substᵀ σ {Γ = Γ} (Λ N) = Λ (substEq (_⊢ _) (substCtx-extsᵗ-⇑ᶜ σ Γ) (substᵀ (extsᵗ σ) N))
...
-- | Term-variable single substitution
...
-- | Type-variable single substitution on terms
_[_]ᵀ : ∀ {Δ} {Γ : Ctx Δ} {A : Type (Δ ,α)}
  → ⇑ᶜ Γ ⊢ A
  → (B : Type Δ)
  → Γ ⊢ A [ B ]ᵗ
_[_]ᵀ {Γ = Γ} {A = A} N B =
  substEq (_⊢ A [ B ]ᵗ) (substCtx-σ₀-⇑ᶜ-cancel Γ B) (substᵀ (σ₀ᵗ B) N)

-----------------------------------------------------------------------------
-- Values and reduction
-----------------------------------------------------------------------------

data Value : ∀ {Δ} {Γ : Ctx Δ} {A : Type Δ} → Γ ⊢ A → Set where
  V-ƛ    : ∀ {Δ} {Γ : Ctx Δ} {A B : Type Δ} {N : Γ , A ⊢ B} → Value (ƛ N)
  V-zero : ∀ {Δ} {Γ : Ctx Δ} → Value (`zero {Γ = Γ})
  V-Λ    : ∀ {Δ} {Γ : Ctx Δ} {A : Type (Δ ,α)} {N : ⇑ᶜ Γ ⊢ A} → Value (Λ N)


data _—→_ : ∀ {Δ} {Γ : Ctx Δ} {A : Type Δ} → Γ ⊢ A → Γ ⊢ A → Set where

  ξ-·₁ : ∀ {Δ} {Γ : Ctx Δ} {A B : Type Δ} {L L′ : Γ ⊢ A ⇒ B} {M : Γ ⊢ A}
    → L —→ L′
      ---------------------
    → L · M —→ L′ · M

  ξ-·₂ : ∀ {Δ} {Γ : Ctx Δ} {A B : Type Δ} {V : Γ ⊢ A ⇒ B} {M M′ : Γ ⊢ A}
    → Value V
    → M —→ M′
      ------------------------
    → V · M —→ V · M′

  ξ-∙ : ∀ {Δ} {Γ : Ctx Δ} {A : Type (Δ ,α)} {M M′ : Γ ⊢ ∀̇ A} {B : Type Δ}
    → M —→ M′
      ------------------------
    → M ∙ B —→ M′ ∙ B

  β-ƛ : ∀ {Δ} {Γ : Ctx Δ} {A B : Type Δ} {N : Γ , A ⊢ B} {W : Γ ⊢ A}
    → Value W
      ---------------------------
    → (ƛ N) · W —→ N [ W ]

  β-Λ : ∀ {Δ} {Γ : Ctx Δ} {A : Type (Δ ,α)} {N : ⇑ᶜ Γ ⊢ A} {B : Type Δ}
      ------------------------------------
    → (Λ N) ∙ B —→ N [ B ]ᵀ
