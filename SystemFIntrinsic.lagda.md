...
types      A ::= ... | Nat | Bool | α | ∀α. A
terms      M ::= ... | zero | suc M | case-nat L M N | true | false | if_then_else L M N
               | Λα. M | M [ A ]  (in the Agda code below, we use _∙_ for type application)
...
-- If ∅ ; ∅ ⊢ M : ∀ α. α -> (α -> α) -> α, then M [ Bool ] true neg —↠ V
-- and M[ Nat ] 1 flip —↠ W and (V, W) ∈ R.
